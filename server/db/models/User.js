...
const UserSuggestion = require('./UserSuggestion');
...
//user can suggest recipe (suggestion parameter is just the url)
User.prototype.suggestRecipe = async function (suggestion) {
  const recipeExists = await Recipe.findOne({
    where: {
      url: suggestion,
    },
  });
  const suggestionExists = await UserSuggestion.findOne({
    where: {
      url: suggestion,
    },
  });
  if (!suggestion || !validator.isURL(suggestion)) {
    const error = Error('Please enter a valid URL');
    throw error;
  }
  if (suggestionExists) {
    const error = Error(
      'You or another user has already suggested this recipe'
    );
    throw error;
  }
  if (recipeExists) {
    const error = Error('Recipe already exists');
    throw error;
  }
  let newSuggestion = await UserSuggestion.create({
    url: suggestion,
    userId: this.id,
  });
